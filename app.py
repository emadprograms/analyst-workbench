...
with tab_runner_eod:
    ...
    # --- FRAGMENT DEFINITION ---
    @st.fragment
    def render_company_updates_fragment(selected_tickers, selected_date, market_news, selected_model):
        ...
        if st.button(f"Run Update for {len(selected_tickers)} Ticker(s)", use_container_width=True):
            if not selected_tickers:
                ...
                if "Data Extraction Summary:" not in stock_summaries_text:
                    ...
                    # In a fragment, st.stop() stops the fragment, not the whole app.
                    st.stop()
    ...
    # --- RENDER FRAGMENT ---
    render_company_updates_fragment(selected_tickers, selected_date, market_news_step3, selected_model)
