...
def update_company_card(
    ...
    **7. `behavioralSentiment` Section (The "Micro" / Today's Analysis):**
        * **`emotionalTone` (The 3-Act Pattern + Proof of Reasoning):**
            ...
            * **MANDATORY TRUTH (SUPPORT/RESISTANCE):** Do NOT claim a support level "held" or was "defended" if the `low` price in `sessions.regular_hours` broke below that mathematical level by more than 0.5%. If the price broke below the level but bounced back later, you MUST describe it as a "washout and reclaim" or a "false breakdown," NOT as holding support.
