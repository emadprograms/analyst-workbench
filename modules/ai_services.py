...
def update_company_card(
    ...
    system_prompt = (
        ...
        "Your job has two distinct analytical tasks: "
        ...
        "You must also maintain the `recentCatalyst` as a 'Governing Narrative', appending new info or carrying over the existing story. "
    ...
    (This is the macro "Headwind" or "Tailwind" for the day. Use this for the 'newsReaction' field.)
    ...
    (This is the established structure, plans, and `keyActionLog` so far. Read this for the 3-5 day context AND to find the previous 'recentCatalyst'.) 
    ...
    **1. `basicContext.recentCatalyst` (The "Governing Narrative"):**
        * This is your first analytical task. You must manage this field as the **cumulative story** of *why* this stock is in play.
    ...
    **2. `technicalStructure` Section (The "Macro" / Zone Analysis):**
    ...
    **3. `behavioralSentiment` Section (The "Micro" / Today's Analysis):**
    ...
    **4. `keyActionLog`:** Write your `todaysAction` log entry *last*, *after* you have done the `behavioralSentiment` analysis, so it uses the same language (e.g., "Committed Buyers stepped away, letting price drift down...").

    **5. `screener_briefing` (The "Pre-Market Cheat Sheet"):**
    ...
    **6. Other Fields (`bias`, `confidence`, `plans`):** Use the `CRITICAL ANALYTICAL RULES` below.
    ...
    * **Confidence:** Calculate `confidence` using this logic:
        * **High:** Today's action *strongly confirmed* the bias AND *respected* MAJOR S/R **OR** it achieved a *decisive, high-volume CLOSE* beyond a MAJOR S/R level.
        * **Medium:** Today's action was mixed, closed *at* a major level (indecision), or a breakout occurred on low volume.
        * **Low:** Today's action *failed* at a level and *reversed* against the bias.
    ...
    * **Volume:** Describe ONLY how volume confirmed or denied the action *at specific levels*.
    ...
    {{
      ...
      "confidence": "Your new confidence (High, Medium, Low)",
