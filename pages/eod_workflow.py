            ...
            try:
                ...
                if not updated_card_str:
                    ...
                    try:
                        ...
                        if not conn_eco_save:
                            st.error("Database Connection Failed. Cannot save Economy Card.")
        ...
        if not selected_tickers:
            ...
            with st.spinner("Running EOD updates for selected companies..."):
                ...
                try:
                    ...
                    if not conn_stock_save:
                        st.error("Database Connection Failed. Cannot save Company Cards.")
                        logger.log("❌ **Error:** Database Connection Failed.")
                        # We can still run the loop to generate content, but we can't save.
                        # Actually, if DB is down, get_company_card will fail too (return defaults).
                        # It's better to stop or skip saving.

                    # --- MERGED STEP 3b: Run AI Loop ---
                    for ticker in selected_tickers:
                        summary = summaries_by_ticker.get(ticker)

                        # --- FIX: Check for explicitly reported errors in summary ---
                        if not summary:
                             logger.log(f"--- ⚠️ Skipping {ticker}: No summary found. ---")
                             failure_list.append(f"{ticker} (Missing Data)")
                             continue

                        if "[ERROR]" in summary:
                            logger.log(f"❌ **Skipping {ticker}:** Data fetch failed. Processor returned error.")
                            # Log the specific error line if possible
                            error_line = next((line for line in summary.split('\n') if "[ERROR]" in line), "Unknown Error")
                            logger.log(f"   -> {error_line}")
                            failure_list.append(f"{ticker} (Fetch Error)")
                            continue
                        ...
                        try:
                            ...
                            if conn_stock_save:
                                ...
                                logger.log(f"⚠️ **Warning:** Generated card for {ticker} but could not save (DB disconnected).")
                                failure_list.append(f"{ticker} (Save Failed)")
                        except json.JSONDecodeError:
